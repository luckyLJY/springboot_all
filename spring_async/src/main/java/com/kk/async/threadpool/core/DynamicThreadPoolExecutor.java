...
public class DynamicThreadPoolExecutor extends ThreadPoolExecutor implements BeanNameAware {
    ...
    protected void beforeExecute(Thread t, Runnable r) {
        ...
        if(monitor!=null){
            ...
            if((double)getQueue().size()/getQueueCapacity()>=0.8){
                monitor.alertCapacity(this);
            }
        }
        LOGGER.info("线程池 {} ：活跃线程数：{} ，待执行任务数 {}",threadPoolName,(double)getActiveCount()/getMaxPoolSize(),(double)getQueue().size()/getQueueCapacity());
